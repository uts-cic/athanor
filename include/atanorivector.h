...
#ifndef atanorivector_h
#define atanorivector_h
...
class Atanorivector;
//This typedef defines a type "ivectorMethod", which expose the typical parameters of a new KiF method implementation
typedef Atanor* (Atanorivector::*ivectorMethod)(Atanor* contextualpattern, short idthread, AtanorCall* callfunc);
//---------------------------------------------------------------------------------------------------------------------


class Atanorivector : public AtanorObject {
    public:
    //We export the methods that will be exposed for our new object
    //this is a static object, which is common to everyone
    //We associate the method pointers with their names in the linkedmethods map
    static Exchanging hmap<unsigned short, ivectorMethod> methods;
	...
	bool isconst;
    //---------------------------------------------------------------------------------------------------------------------
    Atanorivector(AtanorGlobal* g, Atanor* parent = NULL) : AtanorObject(g, parent) {
    ...
    //----------------------------------------------------------------------------------------------------------------------
    Exporting Atanor* Put(Atanor* idx, Atanor* value, short idthread);
    ...
    Atanor* Atom(bool forced = false) {
        if (forced || !protect || reference) {
			Atanorivector* v = globalAtanor->Provideivector();
            Locking _lock(this);
            v->values = values;
            return v;
        }
    ...
    static void AddMethod(AtanorGlobal* g, string name, ivectorMethod func, unsigned long arity, string infos);
    ...
    Atanor* MethodPush(Atanor* contextualpattern, short idthread, AtanorCall* callfunc) {
        ...
        for (size_t i = 0; i < callfunc->Size(); i++)
            values.push_back(callfunc->Evaluate(i, contextualpattern, idthread)->Integer());
        return aTRUE;
    }
    ...
    Atanor* Inverse() {
        Locking _lock(this);
		Atanorivector* vect = globalAtanor->Provideivector();
...
class AtanorIterationivector : public AtanorIteration {
    public:
    ...
    AtanorIterationivector(Atanorivector* v, bool d, AtanorGlobal* g = NULL) : _lock(v), AtanorIteration(d, g) {
        ref = v;
        itx = 0;
    }
...
//---------------------------------------------------------------------------------------------------------------------
class Atanorivectorbuff : public Atanorivector {
public:
	long idx;
	bool used;

	Atanorivectorbuff(long i)  {
		//Do not forget your variable initialisation
		idx = i;
		used = false;
	}
	...
	void Resetreference(short r) {
		...
		if (clr) {
			Locking _lock(globalAtanor->ivectorlock);
			used = false;
			globalAtanor->ivectorempties.push_back(idx);
		}
	}
