...
static size_t call_writing(char *ptr, size_t size, size_t nmemb, Atanorcurl* userdata) {
	int max = size*nmemb;
	string s;
	for (int i = 0; i < max; i++)
		s += ptr[i];

	Atanor* func = userdata->function;
	AtanorCallFunction kfunc(func);

	Atanorstring* kstr = globalAtanor->Providestring(s);
	kstr->Setreference();
...
hmap<short, curlMethod>  Atanorcurl::methods;
hmap<string, string> Atanorcurl::infomethods;
bin_hash<unsigned long> Atanorcurl::exported;

short Atanorcurl::idtype = 0;


//MethodInitialization will add the right references to "name", which is always a new method associated to the object we are creating
void Atanorcurl::AddMethod(AtanorGlobal* global, string name, curlMethod func, unsigned long arity, string infos) {
...
bool Atanorcurl::InitialisationModule(AtanorGlobal* global, string version) {
	methods.clear();
	infomethods.clear();
	exported.clear();

	Init();
	Atanorcurl::idtype = global->Getid("curl");


	Atanorcurl::AddMethod(global, "_initial", &Atanorcurl::MethodInitial, P_ONE, "_initial(obj): catch an object for function call");

	Atanorcurl::AddMethod(global, "proxy", &Atanorcurl::MethodProxy, P_ONE, "proxy(string prox): Proxy connection");
	Atanorcurl::AddMethod(global, "password", &Atanorcurl::MethodPWD, P_TWO, "password(string user, string pwd): Provide the user password");
	Atanorcurl::AddMethod(global, "url", &Atanorcurl::MethodURL, P_ONE | P_TWO, "url(string path): Load a URL\rurl(string pathstring filename): Load a url into a filename");
	Atanorcurl::AddMethod(global, "execute", &Atanorcurl::MethodExecute, P_NONE | P_ONE, "execute(string filename): Execute a curl action filename is optional.");
	Atanorcurl::AddMethod(global, "options", &Atanorcurl::MethodOptions, P_TWO, "options(string option, string parameter): Set the options of the curl object");

	global->newInstance[Atanorcurl::idtype] = new Atanorcurl(global);
	global->RecordMethods(Atanorcurl::idtype,Atanorcurl::exported);
...
Atanor* Atanorcurl::Put(Atanor* index, Atanor* value, short idthread) {
	return this;
}

Atanor* Atanorcurl::Get(Atanor* context, Atanor* index, short idthread) {
	return this;
}



Atanor* Atanorcurl::MethodProxy(Atanor* contextualpattern, short idthread, AtanorCall* callfunc) {
...
Atanor* Atanorcurl::MethodPWD(Atanor* contextualpattern, short idthread, AtanorCall* callfunc) {
	//In our example, we have only two parameters
	//0 is the first parameter and so on...
...
Atanor* Atanorcurl::MethodURL(Atanor* contextualpattern, short idthread, AtanorCall* callfunc) {
	//In our example, we have only two parameters
	//0 is the first parameter and so on...
	...
	if (callfunc->Size() == 1) {
		...
		if (tmp == NULL)
			return globalAtanor->Returnerror("URL(009): Wrong filename", idthread);
...
Atanor* Atanorcurl::MethodExecute(Atanor* contextualpattern, short idthread, AtanorCall* callfunc) {
...
Atanor* Atanorcurl::MethodOptions(Atanor* contextualpattern, short idthread, AtanorCall* callfunc) {
	string option = callfunc->Evaluate(0, contextualpattern, idthread)->String();
	if (curloptions.find(option) == curloptions.end())
		return globalAtanor->Returnerror("URL(031): Unknown option", idthread);
	CURLcode res;
