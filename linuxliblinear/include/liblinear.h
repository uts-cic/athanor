#ifndef atanorliblinear_h
#define atanorliblinear_h
...
class Atanorliblinear : public AtanorObject {
    public:
    ...
    //---------------------------------------------------------------------------------------------------------------------
	Atanorliblinear(AtanorGlobal* g, Atanor* parent = NULL);
    ...
    Atanor* Atom(bool forced=false) {
        if (forced || !protect)
            return new  Atanorliblinear(NULL);
        return this;
    }
	...
	static void AddMethod(AtanorGlobal* g, string name, liblinearMethod func, unsigned long arity, string infos);
	...
	SVMProblem load_short_problem(Atanorfvector* labels, Atanorvector* data, int& error_code) {
		...
		*/
		...
		Atanortreemapif* ktmif;
		Atanortreemapif mapbase;
		Atanor* mymap;
		...
		for (i = 0; i < xprob.l; i++)
		{
			...
			if (mymap->Type() != Atanortreemapif::idtype) {
				...
				AtanorIteration* itr = mymap->Newiteration(false);
	...
	int load_vector_problems(Atanorfvector* labels, Atanorvector* data) {
		int error_code = 0;
	...
	int analysis_problems(INT64* offsets, Atanorvector* kvect, bool training) {
		int i, j;
		INT64 error_code = 0;
		...
		*/
		...
		{
			INT64 base = 0;
			Atanorvector* kvi = globalAtanor->Providevector();
			kvi->values.push_back(globalAtanor->Providefloat(svmproblems[0].prob.y[j]));
			Atanormapif* kval = new Atanormapif;
			kvi->Push(kval);
			kvect->Push(kvi);
			for (i = 0; i < num_srcs; i++)
			{
	...
	Atanor* do_predict_with_file(FILE *input, Atanorvector* output) {

		Atanortreemap* kinfo = NULL;
		if (infos) {
			kinfo = new Atanortreemap;
		...
		Atanorivector* kindexlabels = NULL;
		if (flag_predict_probability) {
			INT64 *labels;

			if (!check_probability_model(model_))
				return globalAtanor->Returnerror("LLN(013): probability output is only supported for logistic regression", globalAtanor->GetaThreadId());

			labels = (INT64 *)malloc(nr_class*sizeof(INT64));
			get_labels(model_, labels);
			prob_estimates = (double *)malloc(nr_class*sizeof(double));
			if (infos) {
				kindexlabels = globalAtanor->Provideivector();
		...
		while (readline(input) != NULL)
		{
			...
			if (flag_predict_probability)
			{
				...
				if (infos == 1 || kindexlabels == NULL) {
					...
					if (infos == 2) {
						Atanortreemapif* kmlabelfloat = new Atanortreemapif;
			...
			else
			{
				...
				output->Push(globalAtanor->Providefloat(predict_label));
				ix++;
			}
		...
		if (infos) {
			if (model_->param.solver_type == L2R_L2LOSS_SVR ||
				...
				kinfo->Push("Mean squared error (regression)", globalAtanor->Providefloat(error / total));
				kinfo->Push("Squared correlation coefficient (regression)",
					globalAtanor->Providefloat(((total*sumpt - sump*sumt)*(total*sumpt - sump*sumt)) / ((total*sumpp - sump*sump)*(total*sumtt - sumt*sumt))));
			}
			else {
				kinfo->Push("Accuracy", globalAtanor->Providefloat(correct / total * 100));
				kinfo->Push("Accuracy (correct)", globalAtanor->Providefloat(correct));
				kinfo->Push("Accuracy (total)", globalAtanor->Providefloat(total));
			}
		}
	...
	Atanor* do_predict(Atanorfvector* klabels, Atanorvector* input, Atanorvector* output) {
		Atanortreemap* kinfo = NULL;
		if (infos) {
			kinfo = new Atanortreemap;
			kinfo->Setreference(output->reference + 1);
		...
		for (INT64 ix = 0; ix < input->values.size(); ix++) {
			...
			if (input->values[ix]->Type() != Atanortreemapif::idtype) {
				if (!input->values[ix]->isContainer())
					return globalAtanor->Returnerror("LLN(011): Wrong map type", globalAtanor->GetaThreadId());
				mapbase.values.clear();
				AtanorIteration* itr = input->values[ix]->Newiteration(false);
				for (itr->Begin(); itr->End() == aFALSE; itr->Next())
					mapbase.values[itr->Keyinteger()] = itr->Valuefloat();
				itr->Release();
				ktmif = &mapbase;
				...
				ktmif = (Atanortreemapif*)input->values[ix];
	...
	Atanor* do_cross_validation() {
		...
		Atanormapsf* kres = new Atanormapsf;
	...
	Atanor* exit_with_help(string o) {
		string msg = "LLN(001): Error with options:";
		msg += o;
		return globalAtanor->Returnerror(msg, globalAtanor->GetaThreadId());
	}

	Atanor* parse_options(Atanor* kmap);

	Atanor* exit_input_error(int line_num) {
		char msg[100];
	...
	Atanor* read_problem(const char *filename) {
		INT64 max_index, inst_max_index, i;
		INT64 elements, j;
		FILE *fp; 
#ifdef WIN32
		...
		if (fp == NULL)
		{
			...
			return globalAtanor->Returnerror(buff, globalAtanor->GetaThreadId());
		}
	...
	Atanor* load_problem(Atanorfvector* labels, Atanorvector* data) {
		if (loaded)
			return globalAtanor->Returnerror("LLN(010): Clean your data first", globalAtanor->GetaThreadId());
	...
	Atanor* MethodInitial(Atanor* contextualpattern, short idthread, AtanorCall* callfunc) {
		...
		Atanor* mfn = callfunc->Evaluate(0, contextualpattern, idthread);
	...
	Atanor* MethodCrossvalidation(Atanor* contextualpattern, short idthread, AtanorCall* callfunc);
	Atanor* MethodClean(Atanor* contextualpattern, short idthread, AtanorCall* callfunc);
	Atanor* MethodTrainingSet(Atanor* contextualpattern, short idthread, AtanorCall* callfunc);
	Atanor* MethodLoadModel(Atanor* contextualpattern, short idthread, AtanorCall* callfunc);
	Atanor* MethodDoPredictFromFile(Atanor* contextualpattern, short idthread, AtanorCall* callfunc);
	Atanor* MethodDoPredict(Atanor* contextualpattern, short idthread, AtanorCall* callfunc);
	Atanor* MethodLoadproblems(Atanor* contextualpattern, short idthread, AtanorCall* callfunc);
	Atanor* MethodAddproblemvectors(Atanor* contextualpattern, short idthread, AtanorCall* callfunc);
	Atanor* MethodComputeidf(Atanor* contextualpattern, short idthread, AtanorCall* callfunc);
	Atanor* MethodNormalize(Atanor* contextualpattern, short idthread, AtanorCall* callfunc);
	Atanor* MethodAnalysisproblems(Atanor* contextualpattern, short idthread, AtanorCall* callfunc);
	Atanor* MethodCleaning(Atanor* contextualpattern, short idthread, AtanorCall* callfunc);
