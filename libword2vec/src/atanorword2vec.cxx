...
public:
	...
	atanorword2vec* current;
	long id;

	localcallthread(atanorword2vec* c, long i) {
		id = i;
		current = c;
	}
...
extern "C" {
    Exporting bool word2vec_InitialisationModule(AtanorGlobal* global, string version) {
		...
		Atanorw2vector::InitialisationModule(global, version);
...
hmap<unsigned short, word2vecItemMethod>  Atanorw2vector::methods;
hmap<string, string> Atanorw2vector::infomethods;
bin_hash<unsigned long> Atanorw2vector::exported;

short Atanorw2vector::idtype = 0;


//MethodInitialization will add the right references to "name", which is always a new method associated to the object we are creating
void Atanorw2vector::AddMethod(AtanorGlobal* global, string name, word2vecItemMethod func, unsigned long arity, string infos) {
...
bool Atanorw2vector::InitialisationModule(AtanorGlobal* global, string version) {
	methods.clear();
	infomethods.clear();
	exported.clear();

	//Each new object has a specific name, which will help recognize it in the code that will exploit word2vec...
	Atanorw2vector::idtype = global->Getid("w2vector");
	...
	//Atanorw2vector::AddMethod(global, "command", &Atanorw2vector::MethodCommand, P_ONE | P_TWO, "command(string test): A sample of how to implement a new method for your object.");
	...
	//by the Declaration method above. When a AtanorCallMethod object is detected during execution, then KiF calls
	...
	Atanorw2vector::AddMethod(global, "_initial", &Atanorw2vector::MethodInitial, P_NONE|P_ONE, "_initial(word2vecitem i): initialize from a word2vectitem");
	Atanorw2vector::AddMethod(global, "dot", &Atanorw2vector::MethodDot, P_ONE, "dot(element): Return the dot product between two words. Element is either a string or a w2vector");
	Atanorw2vector::AddMethod(global, "cosine", &Atanorw2vector::MethodCosine, P_ONE, "cosine(element): Return the cosine distance between two words. Element is either a string or a w2vector.");
	Atanorw2vector::AddMethod(global, "distance", &Atanorw2vector::MethodDistance, P_ONE, "distance(element): Return the distance between two words. Element is either a string or a w2vector.");
	Atanorw2vector::AddMethod(global, "threshold", &Atanorw2vector::MethodThreshold, P_NONE | P_ONE, "threshold(element): Return or set the threshold.");
	Atanorw2vector::AddMethod(global, "norm", &Atanorw2vector::MethodNorm, P_NONE, "norm(element): Return the vector norm.");


	//We need this code, in order to create new instances of our word2vec object... DO NOT ALTER
	global->newInstance[Atanorw2vector::idtype] = new Atanorw2vector(global);
	global->RecordMethods(Atanorw2vector::idtype, Atanorw2vector::exported);
...
bool Atanorword2vec::InitialisationModule(AtanorGlobal* global, string version) {
	...
	//ExecuteMethod, which must have been implemented for our new object (see above again). We also link this new name
	//with a specific method from our new class.
	Atanorword2vec::AddMethod(global, "initialization", &Atanorword2vec::MethodInit, P_ONE, "initialization(map m): Initialization of a word2vec training set");
	Atanorword2vec::AddMethod(global, "trainmodel", &Atanorword2vec::MethodTrainModel, P_NONE | P_ONE, "trainmodel(vector v): Launching the training. If v is not supplied, then the system utilizes the input file given in the initialisation options");
	Atanorword2vec::AddMethod(global, "loadmodel", &Atanorword2vec::MethodLoadModel, P_ONE | P_TWO, "loadmodel(string filename,bool normalize): Loading a model");
	Atanorword2vec::AddMethod(global, "analogy", &Atanorword2vec::MethodAnalogy, P_ONE, "analogy(svector words): Finding analogies for a group of words. Return a fmap");
	Atanorword2vec::AddMethod(global, "accuracy", &Atanorword2vec::MethodAccuracy, P_ONE | P_TWO, "accuracy(vector words,int threshold): Finding accuracies for a vector of many times 4 words. Return a fmap. If threshold is not supplied then its value is 30000");
	Atanorword2vec::AddMethod(global, "distance", &Atanorword2vec::MethodDistance, P_ONE, "distance(svector words): Finding the distance in a vector of words. Return a fmap.");
	Atanorword2vec::AddMethod(global, "vocabulary", &Atanorword2vec::MethodVocabulary, P_NONE, "vocabulary(): Return a itreemap of the vocabulary covered by the training.");
	Atanorword2vec::AddMethod(global, "features", &Atanorword2vec::MethodFeatures, P_NONE, "features(): Return a map of the vocabulary with their feature values.");
...
AtanorIteration* Atanorw2vector::Newiteration(bool direction) {
	//If you do not need an iteration, do:
	return aITERNULL;
}
...
Atanor* Atanorw2vector::Get(Atanor* context, Atanor* idx, short idthread) {
	return this;
}

Atanor* Atanorword2vec::Get(Atanor* context, Atanor* idx, short idthread) {
	if (w2vec == NULL)
		return globalAtanor->Returnerror("W2V(110): Empty model");
...
void atanorword2vec::LaunchTrainModel() {
#ifndef WIN32
...
Atanor* atanorword2vec::init(Atanor* themap) {
	...
	AtanorIteration* itr = themap->Newiteration(false);
	string key;
	for (itr->Begin(); itr->End() == aFALSE; itr->Next()) {
		...
		return globalAtanor->Returnerror(errmsg);
	}

	itr->Release();

	if (read_context_file[0] && read_vocab_file[0] == 0)
		return globalAtanor->Returnerror("W2V(032): Missing vocabulary file");
...
void atanorword2vec::InitUnigramTable() {
	int a, i;
...
void atanorword2vec::ReadWord(char *word, FILE *fin) {
	int a = 0, ch;
	while (!feof(fin)) {
		ch = fgetc(fin);
		if (ch == 13) continue;
...
void *TrainModelThread(void *id) {
	...
	atanorword2vec* current = lcall->current;
...
void *TrainModelThreadVector(void *id) {
	...
	Atanor* inputwords = current->inputwords;
