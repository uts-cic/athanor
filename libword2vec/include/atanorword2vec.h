...
public:
	...
	Atanortreemapsi d_vocab;
	Atanor* inputwords;
#ifdef WIN32
	vector<bool> tobejoined;
#endif
	
	atanorword2vec(Atanorword2vec* m) {
		mother = m;
		output_file[0] = 0;
...
#endif		
		if (fin == NULL) {
			globalAtanor->Returnerror("W2V(001):ERROR: training data file not found!");
			return -1;
		}
		while (1) {
			ReadWord(word, fin);
	...
	void FeedVocabFromVector(Atanor* words) {
		inputwords = words;
	...
	int ReadVocab() {
		...
		if (fin == NULL) {
			globalAtanor->Returnerror("W2V(002): Vocabulary file not found");
			return -1;
		}
		for (a = 0; a < W2VEC_vocab_hash_size; a++) vocab_hash[a] = -1;
		vocab_size = 0;
...
#endif		
			if (fin == NULL) {
				globalAtanor->Returnerror("W2V(003): ERROR: training data file not found!");
				return -1;
			}
	...
	int loadcontext(string vectorfile) {
...
#endif
		if (f == NULL) {
			globalAtanor->Returnerror("W2V(005): Input file not found");
			return -1;
		}
		...
		fscanf_s(f, "%lld", &lsz);
		if (lsz != layer1_size) {
			globalAtanor->Returnerror("W2V(045): Mismatch between context and vocabulary file");
			return -1;
		}
		InitNet();
	...
	int TrainModelFromVector(Atanor* v) {
		if (output_file[0] == 0) return-1;
		starting_alpha = alpha;
	...
	int analogy(Atanor* vwords, Atanormapsf* resmap) {
		if (M == NULL) {
			globalAtanor->Returnerror("W2V(011): Load a model first");
			return -1;
		}

		if (vwords->Size() < 3) {
			globalAtanor->Returnerror("W2V(006): Requires at least two words for analogy");
			return -1;
		}
		...
		for (a = 0; a < mx; a++) {
			...
			if (it == d_vocab.values.end()) {
				globalAtanor->Returnerror("W2V(008): Out of dictionary word!");
				b = 0;
				break;
			}
			b = it->second;
	...
	int accuracy(long long threshold, Atanor* vwords, Atanormapsf* resmap) {
		if (M == NULL) {
			globalAtanor->Returnerror("W2V(011): Load a model first");
			return -1;
		}
		...
		TCN = 0;
		AtanorIteration* itr = vwords->Newiteration(false);
		Atanor* subvect;
		agnostring key;
		...
		for (itr->Begin(); itr->End() == aFALSE; itr->Next()) {
			...
			if (subvect->Size() != 4) {
				globalAtanor->Returnerror("W2V(030): Expecting sub-vectors of 4 elements");
				return -1;
			}
	...
	float dot(float* v1, vector<Atanor*>& v2, float& norm) {
		if (v2.size() != a_size)
			return 0;
	...
	int distance(string wrd, Atanormapsf* resmap) {
	...
	int distance(Atanor* vwords, Atanormapsf* resmap) {
		char bestw[distance_N][max_size];
		char st[100][max_size];
		float dist, len, bestd[distance_N], vec[max_size];
		long long a, b, c, d, cn, bi[100];
...
public:
	...
	Atanor* MethodInit(Atanor* contextualpattern, short idthread, AtanorCall* callfunc) {
	...
	Atanor* MethodTrainModel(Atanor* contextualpattern, short idthread, AtanorCall* callfunc) {
		if (callfunc->Size() == 0) {
		...
		if (kvect->isVectorContainer() == false)
			return globalAtanor->Returnerror("W2V(029): Expecting a vector as input");
	...
	Atanor* MethodAnalogy(Atanor* contextualpattern, short idthread, AtanorCall* callfunc) {
		Atanor* kvect = callfunc->Evaluate(0, contextualpattern, idthread);
		if (kvect->isVectorContainer() == false)
			return globalAtanor->Returnerror("W2V(021): Need a vector as input to analogy");

		Atanormapsf* resmap;
		if (contextualpattern->Type() == Atanormapsf::idtype) {
			resmap = (Atanormapsf*)contextualpattern;
			resmap->Clear();
		}
		else
			resmap = new Atanormapsf;
	...
	Atanor* MethodAccuracy(Atanor* contextualpattern, short idthread, AtanorCall* callfunc) {
		Atanor* kvect = callfunc->Evaluate(0, contextualpattern, idthread);
		if (kvect->isVectorContainer() == false)
			return globalAtanor->Returnerror("W2V(022): Need a vector as input to accuracy");
	...
	Atanor* MethodFeatures(Atanor* contextualpattern, short idthread, AtanorCall* callfunc) {
		...
		if (contextualpattern->Type() == a_map) {
			kmap = (Atanormap*)contextualpattern;
			kmap->Clear();
		}
		else
			kmap = new Atanormap;
	...
	Atanor* MethodVocabulary(Atanor* contextualpattern, short idthread, AtanorCall* callfunc) {
		return &w2vec->d_vocab;
	}

	Atanor* MethodDistance(Atanor* contextualpattern, short idthread, AtanorCall* callfunc) {
		Atanor* kvect = callfunc->Evaluate(0, contextualpattern, idthread);
...
//We create a map between our methods, which have been declared in our class below. See MethodInitialization for an example
//of how to declare a new method.
class Atanorw2vector;
//This typedef defines a type "word2vecMethod", which expose the typical parameters of a new KiF method implementation
typedef Atanor* (Atanorw2vector::*word2vecItemMethod)(Atanor* contextualpattern, short idthread, AtanorCall* callfunc);
//---------------------------------------------------------------------------------------------------------------------
class Atanorw2vector : public AtanorObject {
public:
	//We export the methods that will be exposed for our new object
	//this is a static object, which is common to everyone
	//We associate the method pointers with their names in the linkedmethods map
	...
	//---------------------------------------------------------------------------------------------------------------------
	Atanorw2vector(AtanorGlobal* g = NULL, Atanor* parent = NULL) : AtanorObject(g, parent) {
		//Do not forget your variable initialisation
		w2vreference = NULL;
		vect = NULL;
		vectnorm = NULL;
	...
	//----------------------------------------------------------------------------------------------------------------------
	Atanor* Put(Atanor* e, Atanor* i, short idthread) {
		if (e->Type() != idtype)
			return globalAtanor->Returnerror("W2V(109) : Cannot initialize with this value");

		Atanorw2vector* item = (Atanorw2vector*)e;
		if (item->vect != NULL) {
			value = item->value;
	...
	Atanor* Atom(bool forced = false) {
		if (forced || !protect) {
			Atanorw2vector* item = new Atanorw2vector;
			if (vect != NULL) {
				item->value = value;
	...
	Atanor* Newinstance(short, Atanor* f = NULL) {
		return new Atanorw2vector;
	}

	AtanorIteration* Newiteration(bool direction);

	static void AddMethod(AtanorGlobal* g, string name, word2vecItemMethod func, unsigned long arity, string infos);
	static bool InitialisationModule(AtanorGlobal* g, string version);
	...
	Atanor* MethodInitial(Atanor* contextualpattern, short idthread, AtanorCall* callfunc) {
		...
		Atanor* e = callfunc->Evaluate(0, contextualpattern, idthread);
		if (e->Type() != idtype)
			return globalAtanor->Returnerror("W2V(109) : Cannot initialize with this value");
	...
	//---------------------------------------------------------------------------------------------------------------------
	Atanor* MethodCosine(Atanor* contextualpattern, short idthread, AtanorCall* callfunc) {
		//In our example, we have only two parameters
		//0 is the first parameter and so on...
		if (vect == NULL || norm == 0)
			return aZERO;

		Atanor* kfirst = callfunc->Evaluate(0, contextualpattern, idthread);
		float lastdistance = 0;
		if (kfirst->Type() == idtype) {
			Atanorw2vector* kwrd = (Atanorw2vector*)kfirst;
			if (kwrd->vect == NULL || kwrd->norm == 0)
				return aZERO;
			lastdistance = w2vreference->w2vec->dot(vect, kwrd->vect);
			lastdistance /= norm*kwrd->norm;
			return globalAtanor->Providefloat(lastdistance);
		}
		...
		if (kfirst->Type() == a_ivector) {
			lastdistance = w2vreference->w2vec->dot(vect, ((Atanorivector*)kfirst)->values, wnorm);
			lastdistance /= norm*wnorm;
			return globalAtanor->Providefloat(lastdistance);
		}

		if (kfirst->Type() == a_fvector) {
			lastdistance = w2vreference->w2vec->dot(vect, ((Atanorfvector*)kfirst)->values, wnorm);
			if (wnorm == 0)
				return aZERO;
			lastdistance /= norm*wnorm;
			return globalAtanor->Providefloat(lastdistance);
		...
		if (kfirst->Type() == a_vector) {
			lastdistance = w2vreference->w2vec->dot(vect, ((Atanorvector*)kfirst)->values, wnorm);
			if (wnorm == 0)
				return aZERO;
			lastdistance /= norm*wnorm;
			return globalAtanor->Providefloat(lastdistance);
	...
	Atanor* MethodDistance(Atanor* contextualpattern, short idthread, AtanorCall* callfunc) {
		...
		return globalAtanor->Returnerror("W2V(111): Unknown item");
	}

	Atanor* MethodDot(Atanor* contextualpattern, short idthread, AtanorCall* callfunc) {
		//In our example, we have only two parameters
		//0 is the first parameter and so on...
		Atanor* kfirst = callfunc->Evaluate(0, contextualpattern, idthread);
		float d;
		...
		if (kfirst->Type() == a_ivector) {
			d = w2vreference->w2vec->dot(vect, ((Atanorivector*)kfirst)->values, wnorm);
			return globalAtanor->Providefloat(d);
		}

		if (kfirst->Type() == a_fvector) {
			d = w2vreference->w2vec->dot(vect, ((Atanorfvector*)kfirst)->values, wnorm);
			return globalAtanor->Providefloat(d);
		}

		if (kfirst->Type() == a_vector) {
			d = w2vreference->w2vec->dot(vect, ((Atanorvector*)kfirst)->values, wnorm);
			return globalAtanor->Providefloat(d);
		}
	...
	Atanor* MethodNorm(Atanor* contextualpattern, short idthread, AtanorCall* callfunc) {
		return globalAtanor->Providefloat(norm);
	}

	Atanor* MethodVectorNorm(Atanor* contextualpattern, short idthread, AtanorCall* callfunc) {
	...
	Atanor* MethodThreshold(Atanor* contextualpattern, short idthread, AtanorCall* callfunc) {
		if (callfunc->Size() == 0)
			return globalAtanor->Providefloat(threshold);
	...
	bool initialization(Atanorword2vec* ref) {
		if (vect == NULL) {
			w2vreference = ref;
			vect = new float[max_size];
			vectnorm = new float[max_size];
	...
	Atanor* plus(Atanor* a, Atanor* b, Atanor* context, bool autoself) {
		if (b->Type() != idtype || a->Type() != idtype)
			return aNULL;

		Atanorw2vector* item = (Atanorw2vector*)b;
		...
		Atanorw2vector* base = (Atanorw2vector*)a;
		if (base->w2vreference != item->w2vreference)
			return aNULL;

		Atanorw2vector* res = new Atanorw2vector;
	...
	Atanor* minus(Atanor* a, Atanor* b, Atanor* context, bool autoself) {
		if (b->Type() != idtype || a->Type() != idtype)
			return aNULL;
	...
	Atanor* same(Atanor* a) {
		...
		float lastdistance = 0;
		if (a->Type() == idtype) {
			Atanorw2vector* w = (Atanorw2vector*)a;
	...
	Atanor* Vector(short idthread) {
		Atanorfvector* kvect = new Atanorfvector;
